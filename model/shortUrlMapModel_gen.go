...
var (
	...
	cacheShortUrlMapSurlPrefix = "cache:shortUrlMap:surl:"
)
...
type (
	shortUrlMapModel interface {
		...
		FindOneBySurl(ctx context.Context, surl string) (*ShortUrlMap, error)
	...
	ShortUrlMap struct {
		...
		Lurl     string    `db:"lurl"`      // 长链接
		Md5      string    `db:"md5"`       // 长链接MD5
		Surl     string    `db:"surl"`      // 短链接
	}
)
...
func (m *defaultShortUrlMapModel) Delete(ctx context.Context, id uint64) error {
	...
	shortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheShortUrlMapSurlPrefix, data.Surl)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, shortUrlMapIdKey, shortUrlMapMd5Key, shortUrlMapSurlKey)
	return err
}
...
func (m *defaultShortUrlMapModel) FindOneBySurl(ctx context.Context, surl string) (*ShortUrlMap, error) {
	shortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheShortUrlMapSurlPrefix, surl)
	var resp ShortUrlMap
	err := m.QueryRowIndexCtx(ctx, &resp, shortUrlMapSurlKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
